<template>
  ...
  <!--v if hace una comparacion si el tamaño de pokemon data es 0, si no se ha buscado-->
  <!-- no va a mostrar nada... por que el objeto esta vacio-->
  <main class="main" v-if="Object.entries(pokemonData).length > 0">
    <section class="pokemonCard">
      <div class="nameImage">
        <h1 clas="pokemonName">{{ pokemonData.name }}</h1>
        <!-- :src es para tener imagen dinamica-->
        <img :src="pokemonData.sprites.front_default" :alt="pokemonData.name" srcset="">
      </div>
      <ul>
        <h2>Type: </h2>
